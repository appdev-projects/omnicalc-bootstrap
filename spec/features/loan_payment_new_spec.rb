...
describe "/loan_payment/new" do
  it "has a navbar with the text 'Omnicalc' that has a Bootstrap green background color.",  {:js => true, :points => 1} do
    ...
    expect(red_value).to eq(40),
      "Expected the amount of Red(#{red_value}) in the backgroun-color to be equal 40, but wasn't."
    expect(green_value).to eq(167),
      "Expected the amount of Green(#{green_value}) in the backgroun-color to be equal 167, but wasn't."
    expect(blue_value).to eq(69),
      "Expected the amount of Blue(#{blue_value}) in the backgroun-color to equal 69, but wasn't."
  end
end
